    ...
    function BeginState() {
        ...
        if (bZapped) {
            ...
            if (Level.Game.GameDifficulty < 2.0) {
                ...
                // Hardest difficulty
                // Doubled Fleshpound Rage time for Suicidal and HoE in Balance Round 1
...
state FireChaingun {
    ...
    function AnimEnd(int Channel) {
        if (MGFireCounter <= 0) {
            ...
            if (Controller.Enemy != none) {
                if (
                    Controller.LineOfSightTo(Controller.Enemy) &&
    ...
    function FireMGShot() {
        ...
        if (A != Level) {
            ...
            if(
                KFPlayerReplicationInfo(KFP.PlayerReplicationInfo).ClientVeteranSkill != class'KFVetBerserker' &&
...
simulated function int DoAnimAction(name AnimName) {
    if (
        AnimName=='PoundAttack1' ||
        AnimName=='FPRageAttack' ||
        AnimName=='PoundAttack3' ||
        AnimName=='FireEndMG'
    ) {
...
function TakeDamage(
    ...
    // Corrected issue where only the Base Health is counted toward the FP's Rage in Balance Round 6(second attempt)
    OldHealth = Health;
    ...
    if (
        !bDecapitated &&
        TwoSecondDamageTotal > RageDamageThreshold &&
        !bChargingPlayer &&
        !bZapped &&
...
defaultproperties {
    ControllerClass=class'ControllerZFPRA'
}